...
App({
  onLaunch: function (options) {
    ...
    if (options.query.hasOwnProperty('sharer')) {
      ...
      that.globalData.sharer = options.query.sharer;
    }
    if (options.query.hasOwnProperty('scene')) {
      sharer = decodeURIComponent(options.query.scene);
      that.globalData.sharer = decodeURIComponent(options.query.scene);
    }
    that.login(sharer);
    that.verify();
  },
  ...
  verify() {
    ...
    AV.Cloud.run('getHash', paramsJson).then(function (settings) {
      let {categories=null,sub_categories=null} =settings;
      settings.categories = JSON.parse(categories);
      settings.sub_categories = JSON.parse(sub_categories);
